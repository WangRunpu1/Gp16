...
function topologyToSvg(topology: any, lang: 'zh' | 'en' = 'zh'): string {
  ...
  // connection dots at edge endpoints (on top of lines, under nodes)
  ...
  const nodeEls = nodes.map((n: any) => {
    ...
    // Language-aware label: if report is English but topology label is Chinese, fall back
    ...
    // Truncate long labels (Chinese ~12 chars, English ~24 chars at current font size)
